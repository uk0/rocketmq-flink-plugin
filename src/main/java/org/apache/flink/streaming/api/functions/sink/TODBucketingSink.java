...
public class TODBucketingSink<T>
    ...
    private static Logger LOG = LoggerFactory.getLogger(TODBucketingSink.class);
    ...
    public TODBucketingSink(String basePath) {
     ...
     */
    public TODBucketingSink<T> setFSConfig(Configuration config) {
     ...
     * the {@link FileSystem} for writing.
     */
    public TODBucketingSink<T> setFSConfig(org.apache.hadoop.conf.Configuration config) {
        this.fsConfig = new Configuration();
    ...
    public void invoke(T value) {
        try {
            ...
            LOG.error("Hdfs写内容出现异常！{}:{}", e.getClass(), e.getMessage());
    ...
    private boolean shouldRoll(BucketState<T> bucketState) throws IOException {
        ...
        if (!bucketState.isWriterOpen) {
            shouldRoll = true;
            LOG.debug("TODBucketingSink {} starting new bucket.", subtaskIndex);
        } else {
            long writePosition = bucketState.writer.getPos();
            if (writePosition > batchSize) {
                shouldRoll = true;
                LOG.debug(
                        "TODBucketingSink {} starting new bucket because file position {} is above batch size {}.",
                        subtaskIndex,
                        writePosition,
                        batchSize);
            }
        ...
        synchronized (state.bucketStates) {
            for (Map.Entry<String, BucketState<T>> entry : state.bucketStates.entrySet()) {
                if (entry.getValue().lastWrittenToTime < currentProcessingTime - inactiveBucketThreshold) {
                    LOG.debug("TODBucketingSink {} closing bucket due to inactivity of over {} ms.",
     ...
     * as its predecessor, but with an increased rolling counter (see {@link TODBucketingSink}.
     */
    private void openNewPartFile(Path bucketPath, BucketState<T> bucketState) {
        try {
            ...
            LOG.error("创建Hdfs文件出现异常！{}:{}", e.getClass(), e.getMessage());
            for (StackTraceElement element : e.getStackTrace()) {
                LOG.error("\tat {}", element.toString());
            }
            e.printStackTrace();
    ...
    private void handlePendingInProgressFile(String file, long validLength) {
        if (file != null) {
            ...
            try {
                ...
                LOG.error("Error while restoring TODBucketingSink state.", e);
                throw new RuntimeException("Error while restoring TODBucketingSink state.", e);
            ...
            for (String filename : pendingFilesPerCheckpoint.get(pastCheckpointId)) {
                ...
                try {
                    if (fs.exists(pendingPath)) {
                        LOG.debug("Restoring TODBucketingSink State: Moving pending file {} to final location after complete checkpoint {}.", pendingPath, pastCheckpointId);
                        fs.rename(pendingPath, finalPath);
                    }
                } catch (IOException e) {
                    LOG.error("Restoring TODBucketingSink State: Error while renaming pending file {} to final path {}: {}", pendingPath, finalPath, e);
    ...
    public void restoreState(RollingSink.BucketState state) throws Exception {
        ...
        try {
            ...
            LOG.error("Error while creating FileSystem when restoring the state of the TODBucketingSink.", e);
            throw new RuntimeException("Error while creating FileSystem when restoring the state of the TODBucketingSink.", e);
        }
     ...
     */
    public TODBucketingSink<T> setBatchSize(long batchSize) {
     ...
     */
    public TODBucketingSink<T> setInactiveBucketCheckInterval(long interval) {
     ...
     */
    public TODBucketingSink<T> setInactiveBucketThreshold(long threshold) {
     ...
     */
    public TODBucketingSink<T> setBucketer(Bucketer<T> bucketer) {
     ...
     */
    public TODBucketingSink<T> setWriter(Writer<T> writer) {
     ...
     */
    public TODBucketingSink<T> setInProgressSuffix(String inProgressSuffix) {
     ...
     */
    public TODBucketingSink<T> setInProgressPrefix(String inProgressPrefix) {
     ...
     */
    public TODBucketingSink<T> setPendingSuffix(String pendingSuffix) {
     ...
     */
    public TODBucketingSink<T> setPendingPrefix(String pendingPrefix) {
     ...
     */
    public TODBucketingSink<T> setValidLengthSuffix(String validLengthSuffix) {
     ...
     */
    public TODBucketingSink<T> setValidLengthPrefix(String validLengthPrefix) {
     ...
     */
    public TODBucketingSink<T> setPartPrefix(String partPrefix) {
     ...
     */
    @Deprecated
    public TODBucketingSink<T> disableCleanupOnOpen() {
        return this;
    }
     ...
     */
    public TODBucketingSink<T> setAsyncTimeout(long timeout) {
